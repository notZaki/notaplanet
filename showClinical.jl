...
# ╔═╡ 8096ca94-4e3a-11eb-235d-ad3d35b13a9a
md"""
Visit: 
$(@bind visit Select(["visit1", "visit2"], default = "visit1"))
"""
...
mat = matread("clinical_$(visit).mat");
...
(midx, midy, midz, midt) = round.(Int, [nx, ny, nz, nt] ./ 2)
...
$(@bind z Slider(1:nz; default = midz, show_value = true))
"""
...
let
	...
	for model in allmodels
		map = sqrt.(mat["rss"][model][50:80,40:80,z])
		push!(p, heatmap(map; yflip = true, clim=(0.01, 0.5), title = model))
	end
	plot(p...)
end
...
let models = ("tofts", "extendedtofts", "uptake", "exchange")
	gr(size=(figwidth,figheight), html_output_format=:png)
	allrss = cat([mat["rss"][model][50:80,40:80,z] for model in models]...; dims = 3)
...
let
	...
	conc = ct[x,y,z,:]
	p = scatter(t, conc; xlim, xlabel = "Time [min]", ylabel = "Concentration [mM]", label = nothing, palette = :seaborn_colorblind, figopts...)
	
	ylim = 1.1 .* extrema(conc)
	
	for model in models
        names = collect(keys(fits[model]))
        values = [fits[model][name][x,y,z] for name in names]
        params = NamedTuple{Tuple(Symbol.(names))}(values)    
        if !isnan(values[1])
            fittedcurve = modelfunc[model](; t, ca, params)
            plot!(t, fittedcurve; ylim, xlim, label = model, figopts...)
        end
    end
	p
end
